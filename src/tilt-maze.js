/* =====================================================================
   TILT MAZE — 思想重力場
   ---------------------------------------------------------------------
   The street encounter's four directions no longer *are* the answer; they
   tilt the plane the answer rolls across. Everything the player does is
   still four buttons.

   This file owns the minigame and nothing else. The story around it — who
   the NPC is, what they conclude, what the world does about it — stays in
   index.html and arrives through the callbacks below.

     TiltMaze.story(levelIndex, npc, done)   done({ won, secs, tilts, left })
     TiltMaze.endless(onQuit)
     TiltMaze.isOpen()

   Grown from .scratch/tilt-maze — the engine, the renderer and the levels
   are that prototype's, unchanged.
   ===================================================================== */
window.TiltMaze = (function () {
  'use strict';
  ...
  /* ---------- DOM (all ids are tm-prefixed; the host page owns the rest) ---- */
  const root = $('tmRoot');
  const arena = $('tmArena'), arenaBoard = $('tmBoard'), arenaCap = $('tmCap');
  const clockFill = $('tmClockFill'), clockLabel = $('tmClockLabel'), clockRight = $('tmClockRight');
  const scoreLabel = $('tmScoreLabel'), scoreNum = $('tmScoreNum');
  const whoPane = $('tmWho'), mePane = $('tmMe'), whoFx = $('tmWhoFx'), meFx = $('tmMeFx');
  const npcTok = $('tmNpcTok'), playerTok = $('tmPlayerTok');
  const whoName = $('tmWhoName'), whoTag = $('tmWhoTag'), whoSay = $('tmWhoSay');
  const whoState = $('tmWhoState'), meState = $('tmMeState');
  const heroRow = $('tmHero'), footerHint = $('tmHint');
  const reportOverlay = $('tmReport'), reportCard = $('tmReportCard'), reportNote = $('tmReportNote');
  const dUp = $('tmUp'), dDown = $('tmDown'), dLeft = $('tmLeft'), dRight = $('tmRight');
  const DPAD = { up: dUp, down: dDown, left: dLeft, right: dRight };

  let mode = 'story';        // 'story' | 'endless'
  let currentNPC = null;
  let onDone = null, onQuit = null;
  let open = false;
  let endless = null;
  ...
  /* =====================================================================
     CONTROL — the countdown, the four buttons, the frame loop
     ===================================================================== */
  ...
  function press(dir, on) {
    ...
    if (on && (!open || !field || !field.running || DPAD[dir].disabled)) return;
  ...
  addEventListener('keydown', e => {
    ...
    if (d) { e.preventDefault(); e.stopPropagation(); press(d, true); }
  }, true);
  ...
  /* A held direction that never gets released would be indistinguishable from a
     permanent slope, so releases are caught at the window and on focus loss. */
  let padDir = null;
  ...
  /* =====================================================================
     STORY — one level, one encounter, no retry
     ---------------------------------------------------------------------
     Both people stay on screen for the whole thing, at bust size, breathing.
     The maze is something happening BETWEEN them, never a mini-game that
     replaced them.
     ===================================================================== */
  function bustCell() {
    const P = window.PORT || { H: 112 };
    const h = whoPane.clientHeight || 420;
    return clamp(Math.floor((h * 0.46) / P.H), 2, 5);
  }
  function bust(canvas, defKey, pose) {
    if (typeof window.setPortrait === 'function') window.setPortrait(canvas, defKey, pose, bustCell());
  }

  function story(levelIndex, npc, done) {
    mode = 'story'; currentNPC = npc; onDone = done; onQuit = null;
    open = true;
    root.classList.add('active');
    heroRow.classList.remove('solo');
    reportOverlay.classList.remove('active');
    scoreLabel.textContent = '思想重力場';
    scoreNum.textContent = '第 ' + (levelIndex + 1) + ' 課';
    clockLabel.textContent = '頓悟窗口';
    footerHint.textContent = '按住方向鍵持續傾斜，放開後場地回正　·　球只認得同色的洞';
    sizeFx();

    whoPane.classList.add('on'); mePane.classList.add('on');
    whoName.textContent = npc.name;
    whoTag.textContent = npc.label || '';
    whoTag.style.color = npc.color || 'var(--text-dim)';
    whoSay.textContent = '「' + npc.question + '」';
    whoState.textContent = '注視著你';
    meState.textContent = '思考中';
    bust(npcTok, npc.faction, 'think');
    bust(playerTok, 'player', 'think');
    fx.mood = 'think'; fx.link = 0;

    const def = STORY_LEVELS[levelIndex % STORY_LEVELS.length];
  ...
  function storyWin() {
    ...
    setTimeout(() => finishStory(stats), 950);
  }
  function storyFail() {
    setInputEnabled(false);
    ...
    setTimeout(() => finishStory(stats), 1150);
  }
  function finishStory(stats) {
    showField(false);
    setTimeout(() => {
      close();
  ...
  /* =====================================================================
     ENDLESS — the arcade cabinet in the back of the association
     ===================================================================== */
  ...
  function startEndless(quitCb) {
    mode = 'endless'; onQuit = quitCb || onQuit; onDone = null;
    open = true;
    root.classList.add('active');
    reportOverlay.classList.remove('active');
    ...
    footerHint.textContent = '通關即進下一關，時間到就結束　·　ESC 離開';
    showField(true);
    updateScore();
    nextEndlessLevel();
  }
  function updateScore() {
    scoreNum.textContent = mode === 'endless' ? endless.score.toLocaleString() : scoreNum.textContent;
  }
  ...
  function showReport() {
    ...
    if (isRecord) {
      best.score = endless.score; best.level = endless.level; best.streak = endless.bestStreak;
      saveBest(best);
    }
  ...
  $('tmAgain').addEventListener('click', () => { reportOverlay.classList.remove('active'); startEndless(); });
  $('tmQuit').addEventListener('click', () => { reportOverlay.classList.remove('active'); quitEndless(); });
  $('tmShot').addEventListener('click', () => {
  ...
  function quitEndless() {
    close();
    const cb = onQuit; onQuit = null;
    if (cb) cb();
  }
  addEventListener('keydown', e => {
    if (open && mode === 'endless' && e.key === 'Escape') { e.preventDefault(); quitEndless(); }
  });
  ...
  function close() {
    ...
    reportOverlay.classList.remove('active');
    if (field) field.running = false;
    field = null;
  }

  return {
    story, endless: startEndless, isOpen: () => open, close,
    // dev handle: tune constants or step a level by hand without a rebuild
    _dev: { get field() { return field; }, held, CAM, cam, fx, STORY_LEVELS, genLevel, solvable }
  };
})();
